...
Header CustomClient::ParseHeader(const std::string& msg) {
  Header h;
...
Url CustomClient::ParseUrl(const std::string& url) {
  Url obj_url;
  if (url.find("http://") == 0) {
    obj_url.https = false;
...
int CustomClient::Send(int socket, const std::string& msg) {
  int length = msg.size();
...
int CustomClient::HttpsReceive(SSL* ssl, char buffer[], int length) {
  int num_read = 0;
...
int CustomClient::HttpsSend(const std::string& msg) {
  char* buffer = new char[msg.size() + 1];
  std::copy(msg.begin(), msg.end(), buffer);
  buffer[msg.size()] = '\0';
...
size_t CustomClient::Recv(int socket, char buffer[], int length) {
  if (use_ssl_) {
...
HttpResponse CustomClient::Receive(int socket) {
...
int CustomClient::GetSocket(const Url& url, const char port[]) {
  addrinfo hints;
...
std::string CustomClient::GetRequestMessage(const Url& url) {
...
int CustomClient::InitSSL(const Url& url) {
...
HttpResponse CustomClient::DownloadHttp(const Url& url) {
  use_ssl_ = false;
...
HttpResponse CustomClient::DownloadHttps(const Url& url) {
  use_ssl_ = true;
...
std::string CustomClient::Download(const std::string& url) {
  ...
  for (int i = 0; i < MAX_REDIRECTS; i++) {
    ...
    HttpResponse res;
    if (obj_url.https) {
      res = DownloadHttps(obj_url);
    } else {
      res = DownloadHttp(obj_url);
    }
